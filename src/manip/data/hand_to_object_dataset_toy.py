...
class HandToObjectDataset(Dataset):
    ...
    def __init__(
        ...
        if single_demo or single_object:
            ...
            print(f"Filtered to demo: {single_demo} object: {single_object}, total: {len(self.processed_data)} demonstrations for overfitting")
    ...
    def _setup_full_trajectory_data_from_windows(self):
        ...
        print(f"Setup full trajectory data: {self.full_length} total frames from {len(self.windows)} windows ({self.split} split)")
    ...
    def has_full_trajectory_data(self):
        ...
        return hasattr(self, 'full_length') and hasattr(self, 'left_hand_full') and hasattr(self, 'right_hand_full') and hasattr(self, 'object_motion_full')
    ...
    def __getitem__(self, idx):
        ...
        traj_noisy_norm = to_tensor(self.normalize_data(window_noisy['object'], 'object'))
        
        # Concatenate hand inputs for conditioning        
        condition = torch.cat([left_hand_norm, right_hand_norm, traj_noisy_norm], dim=-1)  # [T, 3*D]
        ...
        return {
            ...
            'hand_raw': torch.cat([left_hand, right_hand], dim=-1),  # [T, 2*D] - left and right hand trajectories
    ...
    def add_noise_data(self, can_window_dict):
        ...
        for param_name in ['left_hand', 'right_hand', 'object',]:
            ...
            transl_noisy = transl + np.random.normal(loc=0.0, scale=self.noise_std_params_dict['object_trans'], size=transl.shape)
            
            rot_mat = rotation_6d_to_matrix_numpy(rot6d)
            # euler angle
            rot_euler = R.from_matrix(rot_mat).as_euler('zxy', degrees=True)
            rot_euler_noisy = rot_euler + np.random.normal(loc=0.0, scale=self.noise_std_params_dict['object_rot'], size=rot_euler.shape)
            rot_mat_noisy = R.from_euler('zxy', rot_euler_noisy, degrees=True).as_matrix()
            rot6d_noisy = matrix_to_rotation_6d_numpy(rot_mat_noisy)

            can_window_dict_noisy[param_name] = np.concatenate([transl_noisy, rot6d_noisy], axis=-1)
