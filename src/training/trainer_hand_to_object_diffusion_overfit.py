...
from src.manip.model.transformer_hand_to_object_diffusion_model import CondGaussianDiffusion
...
def sample_full_trajectory_inpaint(diffusion_model, full_dataset, device, window_size=120, overlap=60):
    """
    ...
    with torch.no_grad():
        for i in range(num_windows):
            ...
            object_motion_init = torch.zeros(1, window_size, full_dataset.pose_dim).to(device)
...
def sample_full_trajectory_sliding_window(diffusion_model, full_dataset, device, window_size=120, overlap=60):
    """
    ...
    with torch.no_grad():
        for i in range(num_windows):
            ...
            if actual_window_size < window_size:
                pad_len = window_size - actual_window_size
                left_hand_window = torch.cat([left_hand_window, torch.zeros(1, pad_len, full_dataset.pose_dim).to(device)], dim=1)
                right_hand_window = torch.cat([right_hand_window, torch.zeros(1, pad_len, full_dataset.pose_dim).to(device)], dim=1)
...
def evaluate_model(diffusion_model, val_dataset, device, num_eval_windows=10):
    ...
    with torch.no_grad():
        ...
        for i in range(eval_windows):
            ...
            tmp_mask = torch.arange(val_dataset.window_size + 1, device=device).expand(1, val_dataset.window_size + 1) < actual_seq_len[:, None].repeat(1, val_dataset.window_size + 1)
            padding_mask = tmp_mask[:, None, :]
...
def train_overfit(opt, device):
    ...
    if opt.use_rerun:
        visualizer = RerunVisualizer(
            ...
            hand_articulations_path=getattr(opt, 'hand_articulations_path', 'data/hand_articulations.pkl')
        )
        # Setup for overfit training
        visualizer.setup_for_overfit_training(full_dataset, object_id=full_dataset.target_object_id)
    else:
    ...
    for epoch in range(total_epochs):
        ...
        for step_in_epoch in range(steps_per_epoch):
            ...
            # Generate padding mask
            actual_seq_len = seq_len + 1
            tmp_mask = torch.arange(opt.window_size+1, device=device).expand(1, opt.window_size+1) < actual_seq_len[:, None].repeat(1, opt.window_size+1)
            padding_mask = tmp_mask[:, None, :]
            ...
            if global_step % 1000 == 0 and global_step > 0:
                ...
                eval_results = evaluate_model(diffusion_model, val_dataset, device, num_eval_windows=5)
    ...
    sampled_motion_full = sample_full_trajectory_inpaint(
        diffusion_model, full_dataset, device, 
        window_size=opt.window_size, 
    ...
    if sampled_motion_full is not None:
        ...
        hand_poses_full = torch.cat([full_dataset.left_hand_full, full_dataset.right_hand_full], dim=-1)
        ...
        print("Skipping full trajectory sampling - not available for this dataset configuration")
    ...
    with open(summary_path, 'w') as f:
        ...
        if opt.use_rerun:
            ...
            f.write(f"Hand articulations: {'Enabled' if opt.use_hand_articulations else 'Disabled'}\n")
...
def parse_opt():
    ...
    # Model parameters
    parser.add_argument('--window_size', type=int, default=120, help='Training window size and model max sequence length')
    ...
    parser.add_argument('--num_epochs', type=int, default=100, help='Number of training epochs (complete passes through dataset)')
    ...
    # Logging parameters
    parser.add_argument('--save_dir', type=str, default='runs/overfit_rerun', help='Directory to save results')
    ...
    parser.add_argument('--exp_name', type=str, default='overfit_with_rerun', help='Experiment name')
    parser.add_argument('--use_wandb', action='store_true', help='Use wandb for logging')
    parser.add_argument('--use_rerun', action='store_true', help='Use Rerun for real-time visualization')
    parser.add_argument('--use_hand_articulations', action='store_true', help='Use hand articulations in visualization (requires articulation data)')
    
    # Visualization parameters
    parser.add_argument('--mano_models_dir', type=str, default='data/mano_models', help='Directory containing MANO model files')
    parser.add_argument('--object_mesh_dir', type=str, default='data/object_meshes', help='Directory containing object mesh files')
    ...
    parser.add_argument('--visualization_frequency', type=int, default=1000, help='How often to save visualization files (steps)')
